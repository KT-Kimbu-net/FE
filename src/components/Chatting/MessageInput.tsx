...
export default function MessageInput(props: TMessageInput) {
  ...
  return (
    <form
      className="flex items-center w-full px-3 gap-10"
    ...
    >
      <textarea
        maxLength={200}
        rows={2}
        className="w-full resize-none outline-none scrollbar-hide rounded-md bg-gray-700 px-3 py-0.5 text-white text-sm"
        ref={props.messageRef}
      />
      <FaArrowRightLong
        className={`w-6 h-6 text-white self-end cursor-pointer`}
